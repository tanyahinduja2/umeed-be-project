...
API_KEY = "29d6afd177121680a1fbe508084887ae84f4a40e0a3227c28420d7fae3681118"
...
def cleanResume(txt):
    ...
    cleanText = re.sub(r'[%s]' % re.escape("""!"#$%&'()*+,-./:;<=>?@[]^_`{|}~"""), ' ', cleanText)  # This line should be safe
...
def fetch_google_jobs(job_title):
    try:
        # Define the API parameters
        api_key = API_KEY
        params = {
            'q': f'{job_title} jobs India',
            'api_key': api_key,
            'engine': 'google_jobs'
        }
        
        # Send a GET request to SERP API
        ...
        if response.status_code == 200:
            ...
            print("Error: Failed to fetch data from SERP API")
            return []
...
def pred():
    ...
    if 'resume' in request.files:
        ...
        # Here, assume these functions are already implemented
        ...
        # Fetch Google Jobs based on the recommended job title
        google_jobs = fetch_google_jobs(recommended_job)
